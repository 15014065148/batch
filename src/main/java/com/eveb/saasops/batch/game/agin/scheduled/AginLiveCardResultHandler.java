...
@Slf4j
@Service
@JobHander(value = "aginLiveCardResultHandler")
public class AginLiveCardResultHandler extends IAginJobHandler {
    @Autowired
    ...
    public AginRequestParameterModel prepareParameter() {
        ...
        parameter.setIndexName(ElasticSearchConstant.AGIN_INDEX_LIVECARDRESULT);
        parameter.setTypeName(ElasticSearchConstant.AGIN_TYPE_LIVECARDRESULT);
        return parameter;
    }
    ...
    protected List<AginRequestParameterModel> prepareApiLine(List<AginRequestParameterModel> parameters, Object[] objects) throws Exception {
        ...
        for (AginRequestParameterModel model : parameters) {
            for (TGmApi api : apiList) {
                ...
                apimodel.setUrl(apimap.get("cardresulturl").toString());
                apimodel.setUsername(apimap.get("cardresultusername").toString());
                apimodel.setPassword(apimap.get("cardresultpassword").toString());
                apimodel.setApi(api);
                list.add(apimodel);
                break;
            }
