...
@Slf4j
@Service
@JobHander(value = "aginHunterJobHandler")
public class AginHunterJobHandler extends IAginJobHandler {

    @Override
    public AginRequestParameterModel prepareParameter() {
        AginRequestParameterModel parameter = new AginRequestParameterModel();
        parameter.setRemotePath("/HUNTER/");
        parameter.setIndexName(ElasticSearchConstant.AGIN_INDEX_HUNTER);
        parameter.setTypeName(ElasticSearchConstant.AGIN_TYPE_HUNTER);
        return parameter;
    }
    ...
    protected List<AginRequestParameterModel> prepareTimePeriod(AginRequestParameterModel parameter, Object[] paramsStrs) throws Exception {
        ...
        startcal.add(Calendar.MINUTE,-ApplicationConstant.CONSTANT_AGIN_HUNTER_DELAY);
        //美东时间差
        ...
        endcal.setTime(date);
        endcal.add(Calendar.MINUTE,-ApplicationConstant.CONSTANT_AGIN_HUNTER_DELAY);
        ...
        if (!str.substring(0, 8).equals(endstr.substring(0, 8))) {
            ...
            model.setFileName(agCutDate(startcal.getTime(), simpleFormat.parse(str.substring(0, 8) + " 23:59:59"), range));
            ...
            modelAfter.setFileName(agCutDate(simpleFormat.parse(endstr.substring(0, 8) + " 00:00:00"), endcal.getTime(), range));
            list.add(model);
            list.add(modelAfter);
        }else
        {
