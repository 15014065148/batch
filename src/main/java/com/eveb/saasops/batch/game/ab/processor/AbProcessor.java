...
import com.eveb.saasops.batch.common.ProcessorAnnotation;
...
import com.eveb.saasops.batch.game.ab.request.AbRequest;
...
@Slf4j
@ProcessorAnnotation("AbProcessor")
public class AbProcessor extends Processor<AbRequestParameter, AbBetLogModel> {
    ...
    protected List<AbBetLogModel> getRequest(GameParameter<Map<String, Object>> parameter) {
        AbRequest abRequest = (AbRequest) SpringContextUtils.getBean("AbRequest");
        List<AbBetLogModel> list = new LinkedList<>();
        try {
            list = abRequest.getBetList(parameter);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return list;
    ...
    @Override
    protected GameBetLogModel<AbBetLogModel> betHandler(GameBetLogModel<AbBetLogModel> model, List<String> prefixList) {
        AbBetLogModel abBetLogModel = model.getBetLogModel();
        if (abBetLogModel.getState() == 1) {
            return null;
        }
    ...
    @Override
    protected RptBetModel rptbetHandler(GameBetLogModel<AbBetLogModel> model) {
        AbBetLogModel abBetLogModel = model.getBetLogModel();
        if (abBetLogModel.getState() == 1) {
            return null;
        }
    ...
    public static String getTimett(String time) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
